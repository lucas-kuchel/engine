...
namespace app {
    GLFWWindowImplementation::GLFWWindowImplementation(const WindowCreateInfo& createInfo)
        ...
        if (!handle_) {
            throw std::runtime_error("Error calling app::GLFWWindowImplementation::GLFWWindowImplementation(): Failed to create window");
        }
    ...
    void GLFWWindowImplementation::setExtent(const data::Extent2D& extent) {
        extent_ = extent;

        glfwSetWindowSize(handle_, extent_.width, extent_.height);
    }

    void GLFWWindowImplementation::setTitle(const std::string& title) {
        title_ = title;

        glfwSetWindowTitle(handle_, title_.c_str());
    }

    void GLFWWindowImplementation::setVisibility(const WindowVisibility& visibility) {
    ...
    const data::Extent2D& GLFWWindowImplementation::getExtent() const {
        return extent_;
    }

    const std::string& GLFWWindowImplementation::getTitle() const {
        return title_;
    }

    const WindowVisibility& GLFWWindowImplementation::getVisibility() const {
        return visibility_;
    }

    std::queue<WindowEvent>& GLFWWindowImplementation::queryEvents() {
        return events_;
    }

    void* GLFWWindowImplementation::getNativeHandle() {
#if defined(PLATFORM_APPLE)
        ...
        throw std::runtime_error("Error calling app::GLFWWindowImplementation::getNativeHandle(): Cannot query backend when not under Cocoa or Win32");

#endif

        throw std::runtime_error("Error calling app::GLFWWindowImplementation::getNativeHandle(): Backend is unsupported");
    }

    void GLFWWindowImplementation::resizeCallback(GLFWwindow* window, int width, int height) {
        auto self = static_cast<GLFWWindowImplementation*>(glfwGetWindowUserPointer(window));
    ...
    void GLFWWindowImplementation::closeCallback(GLFWwindow* window) {
        auto self = static_cast<GLFWWindowImplementation*>(glfwGetWindowUserPointer(window));
    ...
    void GLFWWindowImplementation::focusCallback(GLFWwindow* window, int focused) {
        auto self = static_cast<GLFWWindowImplementation*>(glfwGetWindowUserPointer(window));
    ...
    void GLFWWindowImplementation::iconifyCallback(GLFWwindow* window, int iconified) {
        auto self = static_cast<GLFWWindowImplementation*>(glfwGetWindowUserPointer(window));
    ...
    void GLFWWindowImplementation::keyCallback(GLFWwindow* window, int key, int, int action, int) {
        auto self = static_cast<GLFWWindowImplementation*>(glfwGetWindowUserPointer(window));
    ...
    void GLFWWindowImplementation::mouseButtonCallback(GLFWwindow* window, int button, int action, int) {
        auto self = static_cast<GLFWWindowImplementation*>(glfwGetWindowUserPointer(window));
    ...
    void GLFWWindowImplementation::mousePositionCallback(GLFWwindow* window, double x, double y) {
        auto self = static_cast<GLFWWindowImplementation*>(glfwGetWindowUserPointer(window));
    ...
    void GLFWWindowImplementation::mouseScrollCallback(GLFWwindow* window, double x, double y) {
        auto self = static_cast<GLFWWindowImplementation*>(glfwGetWindowUserPointer(window));
    ...
    Key GLFWWindowImplementation::mapKey(std::int32_t key) {
        switch (key) {
            case GLFW_KEY_A:
                return Key::A;
    ...
    MouseButton GLFWWindowImplementation::mapMouseButton(std::int32_t button) {
        switch (button) {
